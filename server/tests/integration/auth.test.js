...
describe('Auth routes', () => {
  ...
  describe('POST /api/v1/auth/register', () => {
    ...
    test('should return 400 error if schoolId is invalid', async () => {
      newUser.schoolId = 'invalid-id';
  ...
  describe('GET /api/v1/auth/verify-email', () => {
    ...
    test('should return 400 if verify email token is missing', async () => {
      ...
      await request(app).get('/api/v1/auth/verify-email').send().expect(httpStatus.BAD_REQUEST);
    });

    test('should return 200 if verify email token is blacklisted (fallback decodes JWT)', async () => {
      await insertUsers([userOne]);
      const expires = moment().add(config.jwt.verifyEmailExpirationMinutes, 'minutes');
      const verifyEmailToken = tokenService.generateToken(userOne._id, expires);
