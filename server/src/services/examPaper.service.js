...
const fs = require('fs');
const { generateAmcSourceForVersion } = require('../amc/amcSourceGenerator');
...
class ExamPaperService {
  ...
  async generateAllPapers(examId, options = {}) {
    ...
    const versions = await ExamVersion.find({ examId: exam._id }).sort({ versionCode: 1 });
    ...
    const uploadResults = await this._uploadAllToCloudinary(examId, versionPdfs, answerSheet, versions);
    ...
    for (let i = 0; i < versions.length; i++) {
      ...
      const questionsForTex = await Promise.all(
        v.questions.map(async (vq) => {
          const origQ = exam.questionIds.find((q) => q._id.toString() === vq.questionId.toString());
          return {
      ...
      try {
        ...
        // Copy sujet.pdf to output dir
        const targetName = `sujet-${versionCode}.pdf`;
        const targetPath = path.join(outputDir, targetName);

        const copyResult = await amcRunner.exec(`cp '${sujetPdf}' '${targetPath}'`);

        if (copyResult.exitCode !== 0) {
          throw new Error(`Failed to copy sujet.pdf: ${copyResult.stderr}`);
        }
        ...
        if (prepareResult.corrigePdf) {
          ...
          const corrigeTargetPath = path.join(outputDir, corrigeTargetName);

          const corrigeCopyResult = await amcRunner.exec(`cp '${prepareResult.corrigePdf}' '${corrigeTargetPath}'`);

          if (corrigeCopyResult.exitCode === 0) {
            corrigePdfPath = corrigeTargetPath;
          }
        }
  ...
  async _compileAnswerSheetProject(exam, versions, options = {}) {
    ...
    try {
      ...
      exported.calagePath = fs.existsSync(path.join(outputDir, 'calage.xy')) ? path.join(outputDir, 'calage.xy') : null;
    }
  ...
  async _uploadAllToCloudinary(examId, versionPdfs, answerSheet, versions) {
    ...
    for (let i = 0; i < versionPdfs.length; i++) {
      ...
      if (vp.skipped) {
        const existingVersion = await ExamVersion.findOne({ examId, versionCode: vp.versionCode }).select(
          'pdfUrl corrigePdfUrl answerSheetPdfUrl'
        );
    ...
    if (answerSheet.skipped) {
      ...
      const existingVersion = await ExamVersion.findOne({ examId }).select('answerSheetPdfUrl').sort({ versionCode: 1 });
      ...
      const answerSheetBuffer =
        answerSheet.answerSheetPdfPath && fs.existsSync(answerSheet.answerSheetPdfPath)
          ? fs.readFileSync(answerSheet.answerSheetPdfPath)
          : null;

      console.log(
        `[ExamPaper] Answer sheet buffer: ${answerSheetBuffer ? 'exists (' + answerSheetBuffer.length + ' bytes)' : 'null'}`
      );
  ...
  async _compileAnswerSheetWithCalage(projectDir, outputDir, texSource) {
    ...
    const xyDst = path.join(outputDir, 'calage.xy');

    if (calageResult.amcCompiledXyPath) {
      // AMC generated amc-compiled.xy - this is the correct file from calibration
      const copyResult = await amcRunner.exec(`cp '${calageResult.amcCompiledXyPath}' '${xyDst}' && echo 'XY_COPY_OK'`);
      console.log('[ExamPaper] amc-compiled.xy copied to calage.xy', copyResult.stdout.trim());
    } else if (calageResult.calagePath) {
      // Fallback: use calage.xy if amc-compiled.xy not available
      const copyResult = await amcRunner.exec(`cp '${calageResult.calagePath}' '${xyDst}' && echo 'XY_COPY_OK'`);
      console.log('[ExamPaper] calage.xy copied (fallback)', copyResult.stdout.trim());
    } else {
      ...
      const copyResult = await amcRunner.exec(
        `[ -f '${calageFile}' ] && cp '${calageFile}' '${xyDst}' && echo 'XY_COPY_OK' || echo 'XY_FALLBACK_FAIL'`
      );
      console.log('[ExamPaper] fallback .xy generation', copyResult.stdout.trim());
    }
    ...
    if (calageResult.sujetPdf) {
      const copyResult = await amcRunner.exec(`cp '${calageResult.sujetPdf}' '${pdfDst}' && echo 'PDF_COPY_OK'`);
      console.log('[ExamPaper] Answer sheet PDF copied', copyResult.stdout.trim());
    } else {
      // Fall back: try to find any generated PDF
      const fallbackPdf = `${projectDir}/amc-compiled.pdf`;
      const copyResult = await amcRunner.exec(
        `[ -f '${fallbackPdf}' ] && cp '${fallbackPdf}' '${pdfDst}' && echo 'PDF_COPY_OK' || echo 'PDF_NOT_FOUND'`
      );
      console.log('[ExamPaper] answer-sheet fallback PDF', copyResult.stdout.trim());
    }
  ...
  async getAnswerKeyForVersion(examId, versionCode) {
    const version = await ExamVersion.findOne({ examId, versionCode }).populate('questions.questionId', 'content options');
