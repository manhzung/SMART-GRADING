...
function parseTexLayout(texSource) {
  ...
  if (verMatch) {
    ...
    config.versionCode.startX = config.studentId.startX + 
      config.studentId.numDigits * (config.studentId.bubbleSize + config.studentId.spacing);
  }
...
function generateStudentIdTracepos(config) {
  ...
  for (let digit = 1; digit <= studentId.numDigits; digit++) {
    for (let value = 1; value <= 10; value++) {  // 10 options per digit (0-9)
...
function generateVersionCodeTracepos(config) {
  ...
  for (let digit = 1; digit <= versionCode.numDigits; digit++) {
    for (let value = 1; value <= 4; value++) {  // 4 options per digit (1-4)
